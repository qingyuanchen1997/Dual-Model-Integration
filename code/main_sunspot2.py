...
weight_dir = basedir+'/user_data/model_data/resnet18x2_change_c.pt'
checkpoint = torch.load(weight_dir)
state_dict = checkpoint['state_dict']
unParalled_state_dict = {}
for key in state_dict.keys():
    unParalled_state_dict[key.replace("module.", "")] = state_dict[key]
model_features1.load_state_dict(unParalled_state_dict)

weight_dir = basedir+'/user_data/model_data/resnet18x2_change_m.pt'
checkpoint = torch.load(weight_dir)
state_dict = checkpoint['state_dict']
unParalled_state_dict = {}
for key in state_dict.keys():
    unParalled_state_dict[key.replace("module.", "")] = state_dict[key]
model_features2.load_state_dict(unParalled_state_dict)
...
for epoch in range(initial_epoch, num_epoch):
    ...
    if epoch_acc>0.93:
        ...
        }, basedir+'/user_data/model_data/test_alpha.pt')
        print("Saving alpha model!")
        break